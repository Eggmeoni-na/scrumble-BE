...
class ToDoServiceIntegrationTest extends IntegrationTestHelper {
	...
	class findToDos{
		...
		void whenExistsOne_success() {
			...
			ToDoRequest request = new ToDoRequest(LocalDate.now(), LocalDate.now(), "DAILY", 0L, 50L);
		...
		void whenExistDays_success() {
			...
			ToDoRequest request = new ToDoRequest(LocalDate.now().minusDays(1), LocalDate.now(), "DAILY", 0L, 50L);
	...
	class no_offset_paging{
		...
		void whenGreaterThanPageSize_success() {
			...
			int pageSize = 3;
			ToDoRequest request = new ToDoRequest(LocalDate.now().minusDays(1), LocalDate.now(), "DAILY", 0L, pageSize);
